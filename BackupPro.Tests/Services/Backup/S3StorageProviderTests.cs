...
namespace BackupPro.Tests.Services.Backup
{
    public class S3StorageProviderTests
    {
        ...
        private static CredentialProtector CreateProtector()
        {
            ...
            // dependan de un archivo Data/master.key generado en disco (mismo enfoque que
        ...
        private static S3StorageProvider CreateProvider(ApplicationDbContext context) =>
            new(context, CreateProtector(), NullLogger<S3StorageProvider>.Instance);
        ...
        public void StorageType_IsS3()
        {
            ...
            Assert.Equal("S3", provider.StorageType);
        }

        [Fact]
        public async Task SaveBackupAsync_ConfigNotFound_ReturnsFailureAndLogsErrorToHistory()
        {
            using var context = CreateContext();
            var provider = CreateProvider(context);
            ...
            var result = await provider.SaveBackupAsync(
                storageId: 999,
                backupStream: new MemoryStream(new byte[] { 1, 2, 3 }),
                fileName: "backup.bak",
        ...
        [Fact]
        public async Task DeleteBackupAsync_ConfigNotFound_ReturnsFalse()
        {
            using var context = CreateContext();
            var provider = CreateProvider(context);

            var result = await provider.DeleteBackupAsync(storageId: 999, backupPath: "algun/backup.zip", storageFileId: null);

            Assert.False(result);
        }

        [Fact]
        public async Task DownloadBackupAsync_ConfigNotFound_ReturnsFailureWithMessage()
        {
            using var context = CreateContext();
            var provider = CreateProvider(context);

            var (success, stream, errorMessage) = await provider.DownloadBackupAsync(storageId: 999, backupPath: "algun/backup.zip", storageFileId: null);
